...
function attackRoll(attacker, defender, isDefending) {
  const attackerRoll =
    roll(1, 20) + attacker.effectiveDexterity + getAttackBonus(attacker);
  const defenderBonus = isDefending ? 5 : 0;
  const defenderRoll =
    roll(1, 20) + defender.effectiveDexterity + defenderBonus;
...
function getCreatureAction(creature, range, player) {
  ...
  if (rand < 0.85) {
    ...
    rand < 0.95 &&
    creature.currentLifeForce < creature.maxLifeForce * 0.5
  ) {
    return 'defend';
  } else {
