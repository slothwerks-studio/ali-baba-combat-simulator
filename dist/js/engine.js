...
export function initCombat(player, enemy) {
...
export function getCurrentWeapon(combatant) {
...
// Attack action
export function attack(attacker, defender, isDefending) {
    if (!combatState.player || !combatState.enemy)
        return false;
    const attackerName = attacker === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
    const defenderName = defender === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
...
function applyDamage(target, damage) {
    if (!combatState.player || !combatState.enemy)
        return;
    const targetName = target === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
...
function updateCombatantStatus(combatant) {
    if (!combatState.player || !combatState.enemy)
        return;
    const targetName = combatant === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
    // Check unconscious
...
// Defend action
export function defend(defender) {
    if (!combatState.player || !combatState.enemy)
        return;
    const defenderName = defender === combatState.player
        ? combatState.player.name
...
export function tackle(attacker, defender) {
    if (!combatState.player || !combatState.enemy)
        return false;
    const attackerName = attacker === combatState.player
        ? combatState.player.name
...
// Disengage action
export function disengage(attacker, defender) {
    if (!combatState.player || !combatState.enemy)
        return false;
    const attackerName = attacker === combatState.player
        ? combatState.player.name
...
// Run away action
export function runAway(runner) {
    if (!combatState.player || !combatState.enemy)
        return;
    const runnerName = runner === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
...
export function attemptWakeUp(combatant) {
    if (!combatState.player || !combatState.enemy)
        return false;
    const combatantName = combatant === combatState.player
        ? combatState.player.name
        : combatState.enemy.name;
