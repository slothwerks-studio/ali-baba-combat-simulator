...
export function attack(attacker, defender, isDefending) {
  ...
  const defenderTotal =
    defenderRoll + defender.effectiveDexterity + defenderBonus;
  log(
    `Attack roll: ${attackerTotal} (1d20: ${attackerRoll} + DEX: ${attacker.effectiveDexterity}${attackBonus > 0 ? ` + Bonus: ${attackBonus}` : ''})`,
  );
  log(
    `Defense roll: ${defenderTotal} (1d20: ${defenderRoll} + DEX: ${defender.effectiveDexterity}${defenderBonus > 0 ? ` + Defend: ${defenderBonus}` : ''})`,
  );
...
export function getOpponent(combatant) {
  if (!combatState.player || !combatState.enemy) return null;
  return combatant === combatState.player
    ? combatState.enemy
    : combatState.player;
}
